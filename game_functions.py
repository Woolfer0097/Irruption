...
def show_name_input():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if check_name(text) == "unique_error":
        ...
        text_result = set_text(textbox, text, 120)
        ...
        if error_label_flag:
            show_error_label("Такое имя пользователя уже занято")
            error_timer += 1
            if error_timer >= 300:
                error_label_flag = False
        # Отрисовка
...
def show_error_label(error):
    font_text = pygame.font.Font("./data/fonts/thintel.ttf", 48)
    text_result = font_text.render(error, True, pygame.Color("red"))
    error_label.blit(text_result, (90, 260))
    screen.blit(error_label, (184, 83))
...
def frame_generate(hero_):
    ...
    player_frame.blit(text, (200 - text.get_width() // 2, text.get_height() // 2 - 10))
    return player_frame
